...
const NavLink = ({ children, href, mobile, setShowNav }: LinkProps) => {
  ...
  return (
    <Link href={href}>
      <a
        className={`${className} ${mobile ? "text-2xl font-semibold" : "text-lg font-medium mx-4"} border-b-4  hover:border-opacity-100 border-indigo-600`}
        href={href}
        onClick={() => {
          setShowNav(false);
        }}
...
const NavBar = () => {
  ...
  return (
    <nav className="relative z-50 filter drop-shadow-cool h-14 block bg-white flex flex-row font-inter px-3">
      {/* logo container */}
      <div className="flex-auto w-1/3 flex items-center">
        <img src="/_logo.svg" className="w-60 sm:w-60 md:w-40 lg:4/6" width="100%" />
      </div>
      ...
      <div className="flex lg:hidden w-2/3 justify-end overflow-y-hidden">
        <button className="z-10">
          <div onClick={(e) => {
            ...
            <span className={`${mobileNavShown ? "rotate-45 translate-y-3" : "rotate-0"}  z-20 h-1 w-6 my-1 rounded-full bg-black transition ease transform duration-300`}  />
            <span
              className={` ${
      ...
      <div className="lg:hidden block absolute top-0 left-0 w-screen h-full overflow-hidden">
        <div
          className={`${
            mobileNavShown ? "left-0" : "left-full"
          } transition-all duration-500 ease-in-out  absolute filter drop-shadow-cool top-0 w-screen h-full bg-white  `}
        >
          ...
          <div className="mx-2 my-2 px-4 py-2 flex items-center bg-white filter drop-shadow-cool rounded-xl">
            ...
            <input className="ml-4 w-full focus:outline-none" placeholder="Search..."/>
          </div>

          <div className="flex flex-col flex-wrap mx-4">
            <div className="flex-inital w-full my-4">
              <NavLink setShowNav={setMobileNavShown} mobile={true} href="/">Discover</NavLink>
            </div>

            <div className="flex-inital w-full my-4">
              <NavLink setShowNav={setMobileNavShown} mobile={true} href="/upload">Upload</NavLink>
            </div>
          </div>
          ...
          <div>
            {!session && (
              <div className="mt-4 mx-4 flex justify-center flex-wrap">
                <span className="text-lg font-medium text-center w-full flex-initial">You are not logged in</span>
              ...
              <div>
                <div className="mt-4 mx-4 flex items-center flex-wrap">
                  ...
                  <div className="flex justify-between w-11/12">
                    <span className="ml-5 text-xl font-semibold">{session.user?.name}</span>
                    <button className="flex items-center text-xl font-semibold mx-4 transform hover:scale-125 transition duration-300 ease-in-out">Sign Out</button>
                  </div>
                </div>
                <div className="grid xs:grid-cols-2 mx-4 mt-6">
                    <div className="min-w-gridItem mt-4 text-lg">Profile</div>
                    <div className="min-w-gridItem mt-4 text-lg">Upvoted Posts</div>

                    <div className="min-w-gridItem mt-4 text-lg">My Favorites</div>

                    <div className="min-w-gridItem mt-4 text-lg">Account Settings</div>
      ...
      <div className=" hidden lg:flex flex-auto w-1/3 items-center justify-center">
        <NavLink setShowNav={setMobileNavShown} mobile={false} href="/">Discover</NavLink>
        <NavLink setShowNav={setMobileNavShown} mobile={false} href="/upload">Upload</NavLink>
      </div>
      {/* user container */}
